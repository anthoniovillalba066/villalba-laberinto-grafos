...
class Maze:
    ...
    def solve_bfs(self) -> tuple[list[tuple[int, int]], list[list[tuple[int, int]]]]:
        ...
        while queue and not found:
            ...
            for _ in range(level_size):
                r, c = queue.popleft()
                if (r, c) == (er, ec):
                    # Terminamos de vaciar el nivel antes de reconstruir
                    # para que `levels` esté completo para la visualización
                    found = True
                    break
    ...
    def solve_dfs(self) -> tuple[list[tuple[int, int]], list[tuple[int, int]]]:
        ...
        visited: set[tuple[int, int]] = set()  # set para chequeo O(1) en vez de O(n)
        ...
        while stack:
            ...
            for _, nb in self.neighbors(r, c):
                pos = (nb.row, nb.col)
                if pos not in visited and pos not in parent:
                    parent[pos] = (r, c)
                    stack.append(pos)
